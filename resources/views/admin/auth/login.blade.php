...
@section('content')
    <div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
        ...
        {{-- Tampilkan Success Message dari Logout --}}
        @if (session('success'))
        ...
        {{-- PERBAIKAN: Ganti route('auth.login') jadi route('auth.login.process') --}}
        <form action="{{ route('auth.login.process') }}" method="POST" class="mt-4">
            @csrf
            <!-- Form -->
            <div class="form-group mb-4">
                ...
                <div class="input-group">
                    ...
                    <input type="email" class="form-control" placeholder="example@company.com" id="email" name="email" value="{{ old('email') }}" autofocus>
                </div>
            </div>
            <!-- End of Form -->
